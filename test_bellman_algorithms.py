...
# Scope numpy stubbing to avoid leaking into other tests
def _import_bellman_with_mock():
    """
    Temporarily mocks numpy during the import of DEMONSTRATION_bellman_algorithms.
    This ensures the module gets a functional mock for its numpy dependency
    without permanently replacing numpy in sys.modules for the entire test process.
    """
    mock_np = MagicMock()
    mock_np.sqrt = math.sqrt
    mock_np.log = math.log

    # Check if numpy is already present to avoid overwriting a real installation
    original_numpy = sys.modules.get("numpy")
    sys.modules["numpy"] = mock_np

    try:
        import DEMONSTRATION_bellman_algorithms as bellman
        return bellman, mock_np
    finally:
        # Restore sys.modules
...
# Perform the scoped import
bellman_module, np = _import_bellman_with_mock()
bellman_volatility_weight = bellman_module.bellman_volatility_weight
calculate_exit_signal = bellman_module.calculate_exit_signal
